...
abstract class BaseGcsReadChannel implements VectoredSeekableByteChannel {
  ...
  public void readVectored(List<GcsObjectRange> ranges, IntFunction<ByteBuffer> allocate)
    ...
    for (GcsObjectCombinedRange combinedRange : combinedRanges) {
      try {
        executorService.submit(() -> readCombinedRange(combinedRange, allocate));
      } catch (Exception e) {
