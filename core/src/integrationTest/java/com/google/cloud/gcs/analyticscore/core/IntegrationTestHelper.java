...
public class IntegrationTestHelper {
    ...
    public static void uploadSampleParquetFilesIfNotExists() throws IOException {
        ...
        if (!IntegrationTestHelper.objectPresentInBucket(TPCDS_CUSTOMER_SMALL_FILE)) {
            logger.info("Uploading small size parquet files to Google Cloud Storage");
            File generatedFile = writer.createSampleParquetFile(100000, TPCDS_CUSTOMER_SMALL_FILE);
            uploadFileToGcsAndDelete(generatedFile);
        }
        if (!IntegrationTestHelper.objectPresentInBucket(TPCDS_CUSTOMER_MEDIUM_FILE)) {
            logger.info("Uploading medium size parquet files to Google Cloud Storage");
            File generatedFile = writer.createSampleParquetFile(1000000, TPCDS_CUSTOMER_MEDIUM_FILE);
            uploadFileToGcsAndDelete(generatedFile);
        }
        if (!IntegrationTestHelper.objectPresentInBucket(TPCDS_CUSTOMER_LARGE_FILE)) {
            logger.info("Uploading large size parquet files to Google Cloud Storage");
            File generatedFile = writer.createSampleParquetFile(10000000, TPCDS_CUSTOMER_LARGE_FILE);
            uploadFileToGcsAndDelete(generatedFile);
        }
    }
    ...
    private static void uploadFileToGcsAndDelete(File file) throws IOException {
        try {
