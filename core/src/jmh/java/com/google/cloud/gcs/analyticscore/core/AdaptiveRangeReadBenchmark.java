...
public class AdaptiveRangeReadBenchmark {
    ...
    private void runBenchmark(AdaptiveRangeReadState state, String requestedSchema, URI fileUri) throws IOException {
       GcsFileSystemOptions options = GcsFileSystemOptions.createFromOptions(
                Map.of(
                        "gcs.analytics-core.file-access-pattern", state.fileAccessPattern,
                        "gcs.analytics-core.min-range-request-size-bytes", state.minRangeRequestSize,
                        "gcs.analytics-core.inplace-seek-limit-bytes", state.inplaceSeekLimit,
                        "gcs.analytics-core.adaptive-range-read-enabled", String.valueOf(state.adaptiveRangeReadEnabled),
                        "gcs.channel.read.vectored.enabled", String.valueOf(state.enableVectoredRead)
                ),
                "gcs."
        );
